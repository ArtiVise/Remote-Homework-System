            ...
            ind.GetListSubjects().then(subjects => {
                if (req.query.subjects === undefined) {
                    ind.GetListTask(subjects[0].id,req.session.User_ID).then(tasks => {
                            res.render('student', {StudentName: req.session.Name,
                                subjects: subjects,
                                body: tasks}); //Рендеринг страницы со списком заданий, если предмет не выбран
                        }
                    )
                } else {
            ...
            ind.GetTask(req.session.IdTask).then(tasks => {
                if(tasks !== '') {
                    ind.GetTaskFiles(req.session.IdTask).then(files => {
                        let filelink = '1';
                        if(files === '') { filelink = ''}else{filelink = files[0].FileLink}
        ...
        ind.GetListSubjects().then(subjects => {
            if (req.query.subjects === undefined) {
                ind.GetListDecisions(subjects[0].id,-1).then(tasks => {
                        res.render('Teacher', {TeacherName: req.session.Name,
                            subjects: subjects,
                            body: tasks});
                    }
                )
